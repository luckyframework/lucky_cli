...
describe "Initializing a new web project" do
  it "creates a full web app successfully" do
    ...
    should_run_successfully "crystal run src/lucky.cr -- init test-project test-project"
  ...
  it "creates an api only web app successfully" do
    ...
    should_run_successfully "crystal run src/lucky.cr -- init test-project test-project --api"
    compile_and_run_specs_on_test_project
  end
  ...
  it "creates an api only app without auth" do
    ...
    should_run_successfully "crystal run src/lucky.cr -- init test-project test-project --api --no-auth"
    compile_and_run_specs_on_test_project
  end
  ...
  it "creates a full app without auth" do
    ...
    should_run_successfully "crystal run src/lucky.cr -- init test-project test-project --no-auth"
    compile_and_run_specs_on_test_project
  end
  ...
  it "does not create project if directory with same name already exist" do
    ...
    Process.run(
      "crystal run src/lucky.cr -- init test-project test-project",
      output: output,
      shell: true
    )
  ...
  it "does not create project if project name is not a valid project name" do
    output = IO::Memory.new
    Process.run(
      "crystal run src/lucky.cr -- init 'test project'",
      env: ENV.to_h,
      output: output,
      shell: true
    )
